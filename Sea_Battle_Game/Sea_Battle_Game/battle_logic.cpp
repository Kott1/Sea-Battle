...
bool Player::Shoot(Player* target, Vec2 Pos)
{
	...
	OnShipHitContext context;
	context.attackerId = this->id;
	context.hittedShip = ship;

	if (ship->health <= 0) {
		target->totalShipCount--;
		EventManager::getSingleton()->triggerEvent(OnShipDestroy, &context);
	}
	else
	{
		EventManager::getSingleton()->triggerEvent(OnShipHit, &context);
	}

	if (target->totalShipCount == 1)
	{
		OnPlayerWinContext WinContext;
		WinContext.winerId = this->id;
		EventManager::getSingleton()->triggerEvent(OnPlayerWin, &WinContext);
	}
